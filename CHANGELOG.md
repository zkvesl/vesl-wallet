...
- **`src/slip10_conformance.rs` — the frozen cross-implementation KAT.** Pins the master key, a non-hardened child and a hardened child against `nockchain-wallet` output. Before it, every derivation test was relational, which is exactly the shape in which a SLIP-10-*shaped* but non-conforming construction passes everything while deriving the wrong keys. ⚠️ **Wrong, not invalid** — the chain never checks how a key was derived (`slip10.hoon` is imported by the wallet app and its tests, by nothing on a consensus path), so a non-conforming derivation still signs and spends normally. What it breaks is **interoperability and recovery**: the same seed phrase in `nockchain-wallet` or iris yields a different key, so refunds and imports land somewhere their owner cannot reach. The hardened vector requires one pass through the retry branch, so that path is pinned rather than merely present; the non-hardened vector is the only thing anywhere pinning the Cheetah limb order.
